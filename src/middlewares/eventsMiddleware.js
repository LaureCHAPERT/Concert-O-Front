...
const eventsMiddleware = (store) => (next) => (action) => {
  switch (action.type) {
    case FETCH_EVENTS_BY_GENRE:
      ...
      axios.get(`http://jeremy-bruguier.vpnuser.lan:8000/api/genre/${action.id}/events`)
        .then((response) => {
    ...
    case FETCH_DETAIL_EVENT:
      ...
      axios.get(`http://jeremy-bruguier.vpnuser.lan:8000/api/event/${action.id}`)
        .then((response) => {
    ...
    case FETCH_EVENTS_ON_HOMEPAGE:
      // We send request to the API in order to get an event detail
      axios.get('http://jeremy-bruguier.vpnuser.lan:8000/api/event/home')
        .then((response) => {
          // console.log(response.data);
    ...
    case FETCH_EVENTS_BY_REGION:
      // We send request to the API in order to get an event list filtered by genres
      axios.get(`http://jeremy-bruguier.vpnuser.lan:8000/api/region/${action.id}/events`)
        .then((response) => {
    ...
    case FETCH_EVENTS_WITH_SEARCHBAR:
      // We send request to the API in order to get an event list filtered by genres
      axios.get(`http://jeremy-bruguier.vpnuser.lan:8000/api/event/region/${action.regionId}/genre/${action.genreId}`)
        .then((response) => {
          console.log('response', response.data);
    ...
    case FETCH_ALL_EVENTS:
      // We send request to the API in order to get an event list filtered by genres
      axios.get('http://jeremy-bruguier.vpnuser.lan:8000/api/event')
        .then((response) => {
          console.log('response', response.data);
          store.dispatch(saveAllEvents());
        })
        .catch(() => {
          console.log('coucou');
        });
